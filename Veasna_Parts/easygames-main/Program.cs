...
// TEMP for assignment demo: always use the stub email service
builder.Services.AddScoped<IEmailService, NullEmailService>();

// --- EMAIL DI: use real SMTP only when fully configured, else use stub ---
bool smtpConfigured =
    !string.IsNullOrWhiteSpace(builder.Configuration["Smtp:Host"]) &&
    !string.IsNullOrWhiteSpace(builder.Configuration["Smtp:Username"]) &&
